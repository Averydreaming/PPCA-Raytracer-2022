...
impl Vec3 {
    pub fn refract(uv: Vec3, n: Vec3, etai_over_etat: f64) -> Vec3 {
        let mut cos_theta = f64::min(Vec3::dot(-uv, n), 1.0);
        let mut r_out_perp = (uv + n * cos_theta) * etai_over_etat;
        let mut r_out_parallel = -n * (f64::sqrt((1.0 - r_out_perp.length_squared()).abs()));
